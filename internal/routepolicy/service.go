...
func NewService(repository *Repository, manager *Manager, compiler *platformpolicy.Compiler, operations operationlog.Recorder, security securitylog.Recorder, logger *slog.Logger) *Service {
	return &Service{repository: repository, manager: manager, compiler: compiler, operations: operations, security: security, logger: logger}
}
...
func (s *Service) Set(ctx context.Context, input SetInput) (Detail, error) {
	...
	err = s.record(ctx, input, started, err)
	if err != nil {
		return Detail{}, err
	}
...
func (s *Service) record(ctx context.Context, input SetInput, started time.Time, resultErr error) error {
	requestID, _ := requestid.FromContext(ctx)
	entry := operationlog.Entry{Operation: "route_policy.set", ResourceType: "route_policy", ResourceID: input.RouteID, Source: "application-service", Protocol: "internal", Request: map[string]any{"route_id": input.RouteID, "status": input.Status, "expected_version": input.ExpectedVersion}, RequestID: requestID, Duration: time.Since(started), Succeeded: resultErr == nil}
	if resultErr != nil {
		entry.ErrorMessage = resultErr.Error()
	}
	if s.operations != nil {
		if err := s.operations.Record(ctx, entry); err != nil && resultErr == nil {
			return apperror.Unavailable("operation log unavailable", err)
		}
	}
	if s.security != nil {
		securityEntry := securitylog.Entry{EventType: "route_policy_changed", SubjectType: "route_policy", SubjectID: input.RouteID, RequestID: requestID, Succeeded: resultErr == nil, Metadata: map[string]any{"status": input.Status, "expected_version": input.ExpectedVersion}}
		if resultErr != nil {
			securityEntry.ErrorMessage = resultErr.Error()
		}
		if err := s.security.Record(ctx, securityEntry); err != nil && resultErr == nil && s.security.FailClosed() {
