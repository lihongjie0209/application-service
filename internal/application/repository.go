...
func (r *SQLRepository) UpdateApplication(ctx context.Context, e sqlx.ExtContext, v Application, expected int64) error {
	res, err := e.ExecContext(ctx, r.db.Rebind(`UPDATE applications SET name=?,description=?,icon=?,default_route=?,sort_order=?,status=?,metadata_json=?,version=version+1,updated_at=?,updated_by=? WHERE id=? AND version=? AND deleted_at IS NULL`), v.Name, v.Description, v.Icon, v.DefaultRoute, v.SortOrder, v.Status, v.MetadataJSON, v.UpdatedAt, v.UpdatedBy, v.ID, expected)
	return stale(res, err)
}
func (r *SQLRepository) GetApplication(ctx context.Context, id string) (Application, error) {
	var v Application
	err := r.db.GetContext(ctx, &v, r.db.Rebind(`SELECT `+applicationColumns+` FROM applications WHERE id=? AND deleted_at IS NULL`), id)
...
func (r *SQLRepository) GetMenu(ctx context.Context, id string) (Menu, error) {
	var v Menu
	err := r.db.GetContext(ctx, &v, r.db.Rebind(`SELECT `+menuColumns+` FROM application_menu_drafts WHERE id=? AND deleted_at IS NULL`), id)
	if errors.Is(err, sql.ErrNoRows) {
		err = ErrNotFound
	}
	return v, err
...
func (r *SQLRepository) UpsertMenu(ctx context.Context, e sqlx.ExtContext, v Menu, expected int64) error {
	...
	res, err := e.ExecContext(ctx, r.db.Rebind(`UPDATE application_menu_drafts SET parent_id=?,menu_code=?,menu_type=?,name=?,i18n_key=?,route=?,component=?,icon=?,external_url=?,permission_code=?,permission_scope=?,sort_order=?,visible=?,status=?,version=version+1,updated_at=?,updated_by=? WHERE id=? AND version=? AND deleted_at IS NULL`), v.ParentID, v.Code, v.Type, v.Name, v.I18nKey, v.Route, v.Component, v.Icon, v.ExternalURL, v.PermissionCode, v.PermissionScope, v.SortOrder, v.Visible, v.Status, v.UpdatedAt, v.UpdatedBy, v.ID, expected)
	return stale(res, err)
}
func (r *SQLRepository) DeleteMenu(ctx context.Context, e sqlx.ExtContext, id string, expected int64, now time.Time, actor string) error {
	res, err := e.ExecContext(ctx, r.db.Rebind(`UPDATE application_menu_drafts SET status='deleted',deleted_at=?,deleted_by=?,version=version+1,updated_at=?,updated_by=? WHERE id=? AND version=? AND deleted_at IS NULL`), now, actor, now, actor, id, expected)
	return stale(res, err)
}
func (r *SQLRepository) ListDraftMenus(ctx context.Context, appID string) ([]Menu, error) {
	items := []Menu{}
	err := r.db.SelectContext(ctx, &items, r.db.Rebind(`SELECT `+menuColumns+` FROM application_menu_drafts WHERE application_id=? AND deleted_at IS NULL ORDER BY parent_id,sort_order,id`), appID)
	return items, err
}
...
func (r *SQLRepository) GetRelease(ctx context.Context, appID string, n int64) (MenuRelease, []Menu, error) {
	var rel MenuRelease
	err := r.db.GetContext(ctx, &rel, r.db.Rebind(`SELECT id,application_id,release_number,status,comment,version,created_at,updated_at,created_by,updated_by FROM application_menu_releases WHERE application_id=? AND release_number=? AND deleted_at IS NULL`), appID, n)
	if errors.Is(err, sql.ErrNoRows) {
		err = ErrNotFound
	}
	if err != nil {
...
func (r *SQLRepository) SetPublishedRelease(ctx context.Context, e sqlx.ExtContext, id string, n, expected int64, now time.Time, actor string) error {
	res, err := e.ExecContext(ctx, r.db.Rebind(`UPDATE applications SET published_release=?,version=version+1,updated_at=?,updated_by=? WHERE id=? AND version=? AND deleted_at IS NULL`), n, now, actor, id, expected)
	return stale(res, err)
}
func (r *SQLRepository) GetGrant(ctx context.Context, tenantID, appID string) (Grant, error) {
	var v Grant
	err := r.db.GetContext(ctx, &v, r.db.Rebind(`SELECT `+grantColumns+` FROM tenant_application_grants WHERE tenant_id=? AND application_id=? AND deleted_at IS NULL`), tenantID, appID)
	if errors.Is(err, sql.ErrNoRows) {
		err = ErrNotFound
	}
	return v, err
...
func (r *SQLRepository) UpdateGrant(ctx context.Context, e sqlx.ExtContext, v Grant, expected int64) error {
	res, err := e.ExecContext(ctx, r.db.Rebind(`UPDATE tenant_application_grants SET status=?,valid_from=?,valid_until=?,source=?,entitlements_json=?,version=version+1,updated_at=?,updated_by=? WHERE id=? AND version=? AND deleted_at IS NULL`), v.Status, v.ValidFrom, v.ValidUntil, v.Source, v.EntitlementsJSON, v.UpdatedAt, v.UpdatedBy, v.ID, expected)
	return stale(res, err)
}
...
func (r *SQLRepository) BatchGrants(ctx context.Context, tenantID string, applicationIDs []string) ([]Grant, error) {
	query, args, err := sqlx.In(`SELECT `+grantColumns+` FROM tenant_application_grants WHERE tenant_id=? AND application_id IN (?) AND deleted_at IS NULL ORDER BY id`, tenantID, applicationIDs)
	if err != nil {
		return nil, err
	}
	items := []Grant{}
...
func (r *SQLRepository) ListActiveGrantsByApplication(ctx context.Context, e sqlx.ExtContext, appID string, at time.Time) ([]Grant, error) {
	items := []Grant{}
	err := sqlx.SelectContext(ctx, e, &items, r.db.Rebind(`SELECT `+grantColumns+` FROM tenant_application_grants WHERE application_id=? AND status='active' AND deleted_at IS NULL AND valid_from<=? AND (valid_until IS NULL OR valid_until>?) ORDER BY tenant_id`), appID, at, at)
	return items, err
}
func (r *SQLRepository) BatchActiveGrants(ctx context.Context, tenantID string, ids []string, at time.Time) (map[string]bool, error) {
	...
	q, args, err := sqlx.In(`SELECT application_id FROM tenant_application_grants WHERE tenant_id=? AND application_id IN (?) AND status='active' AND deleted_at IS NULL AND valid_from<=? AND (valid_until IS NULL OR valid_until>?)`, tenantID, ids, at, at)
	if err != nil {
		return nil, err
	}
	var active []string
