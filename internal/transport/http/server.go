...
import (
	...
	docs "github.com/lihongjie0209/application-service/docs"
	...
	"github.com/lihongjie0209/application-service/internal/ratelimit"
...
func NewServer(lc fx.Lifecycle, cfg config.Config, handler *Handler, authService *auth.Service, authorizer platformauthz.Authorizer, limiter *ratelimit.Limiter, idempotencyManager *idempotency.Manager, metrics *observability.Metrics, tracing *observability.Tracing, logger *slog.Logger) (*http.Server, error) {
	...
	api.POST("/applications/create", handler.CreateApplication)
	api.POST("/applications/update", handler.UpdateApplication)
	api.POST("/applications/get", handler.GetApplication)
	api.POST("/applications/list", handler.ListApplications)
	api.POST("/applications/menus/upsert", handler.UpsertMenu)
	api.POST("/applications/menus/get", handler.GetMenu)
	api.POST("/applications/menus/delete", handler.DeleteMenu)
	...
	api.POST("/applications/tenant-grants/get", handler.GetTenantApplicationGrant)
