...
func TestIdempotencyExecutionCompletesAndReplaysApplicationMutation(t *testing.T) {
	t.Parallel()
	...
	router.Use(RequestID(), func(c *gin.Context) {
		...
		c.Next()
	}, IdempotencyExecution(manager, []string{"/api/v1/applications/menus/publish"}, logger))
	router.POST("/api/v1/applications/menus/publish", func(c *gin.Context) { calls++; OK(c, gin.H{"version": 2}) })
	recorder := httptest.NewRecorder()
	router.ServeHTTP(recorder, httptest.NewRequest(http.MethodPost, "/api/v1/applications/menus/publish", strings.NewReader(`{"application_id":"app-1"}`)))
	...
	replay := httptest.NewRequest(http.MethodPost, "/api/v1/applications/menus/publish", strings.NewReader(`{"application_id":"app-1"}`))
...
func TestIdempotencyExecutionBypassesApplicationQueries(t *testing.T) {
	t.Parallel()
	manager := &fakeIdempotencyManager{decision: idempotency.Decision{State: idempotency.StateConflict}}
	for _, route := range []string{"/api/v1/applications/list", "/api/v1/applications/navigation/get", "/api/v1/applications/tenant-grants/batch-check"} {
		t.Run(route, func(t *testing.T) {
			...
			router.Use(func(c *gin.Context) {
				c.Request = c.Request.WithContext(idempotency.WithContext(c.Request.Context(), "operation-1"))
				c.Next()
			}, IdempotencyExecution(manager, []string{"/api/v1/applications/menus/publish"}, slog.New(slog.NewTextHandler(io.Discard, nil))))
