...
func TestMetrics_Collect(t *testing.T) {
	...
	metrics.ObserveRoutePolicyRefresh("redis", "success", time.Now())
	families, err := metrics.registry.Gather()
	if err != nil {
		t.Fatal(err)
	}
	want := map[string]bool{"http_requests_total": false, "http_request_duration_seconds": false, "cron_runs_total": false, "route_policy_refresh_total": false, "route_policy_refresh_duration_seconds": false}
