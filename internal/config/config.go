...
func setDefaults(v *viper.Viper) {
	v.SetDefault("app.name", "application-service")
	...
	v.SetDefault("database.schema", "application")
	...
	v.SetDefault("jwt.issuer", "application-service")
	...
	v.SetDefault("auth.audience", "application-service")
	...
	v.SetDefault("migration.table", "application_schema_migrations")
...
func (c Config) Validate() error {
	...
	if (c.OperationLog.Enabled || c.SecurityLog.Enabled) && !c.EventBus.Enabled {
		return errors.New("enabled operation_log or security_log requires event_bus")
	}
	...
	if c.SecurityLog.Enabled && (strings.TrimSpace(c.SecurityLog.Subject) == "" || c.SecurityLog.MaxPayloadBytes < 256 || c.SecurityLog.MaxPayloadBytes > 1<<20 || len(c.SecurityLog.HashKey) < 32) {
		return errors.New("enabled security_log requires subject, max_payload_bytes between 256 bytes and 1 MiB, and hash_key of at least 32 bytes")
	}
