...
Platform application catalog, menu publication, and tenant-application grant service. It uses Gin, gRPC, Uber Fx, PostgreSQL/MySQL-compatible migrations, Redis distributed locks, NATS JetStream outbox delivery, JWT/JWKS authentication, and optimistic locking.
...
```bash
make docker-build
docker inspect application-service:$(git describe --tags --always --dirty) \
...
After unit and integration tests pass, CI builds `linux/amd64` and `linux/arm64` images and publishes `main`, `sha-*`, and semantic-version tags to `ghcr.io/lihongjie0209/application-service`. Images include Git metadata, an SBOM, and build provenance. Pull requests build both architectures without publishing.
...
kubectl create secret generic application-service --namespace platform \
...
kubectl wait --namespace platform --for=condition=complete job/application-service-migrate --timeout=5m
